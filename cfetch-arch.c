...
struct sysinfo {
    ...
    int ramused, ramtotal, pacman, flatpak, snap, day, hour, min, sec;
} sysinfo;
...
void getos() {
    FILE *os = popen("cat /etc/*-release | grep 'PRETTY_NAME=' | cut -d '\"' -f2", "r");
...
int main() {
    ...
    printf("%s  /.\\`      \\`.\\  PKGS%s   pacman: %d, flatpak: %d, snap: %d%s\n", bold, reset, sysinfo.pacman, sysinfo.flatpak, sysinfo.snap, reset);
