...
int day, hour, min, sec, ramused, ramtotal, kiss;
...
void detectPackages() {
    FILE *kisspac = popen("kiss list | wc -l 2>/dev/null", "r");

    fscanf(kisspac, "%d", &kiss);
    fclose(kisspac);
}
...
int main() {
    ...
    printf("%s  \\|/-___\\|/  PKGS%s   %d%s\n", bold, reset, kiss, reset);
